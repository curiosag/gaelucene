...
public class GAEIndexReaderPool {
	// global logger
    private static final Logger log = Logger.getLogger(GAEIndexReaderPool.class.getName());
	...
	public void showPoolStats(OutputStream out) throws IOException {
		...
		for (Iterator<String> iter = cachedReaders.keySet().iterator(); iter.hasNext();) {
			...
			if (indexReader == null) {
				...
				statsInfo.append("\trefCount=").append(refTotalCount).append(
						"/").append(refCurCount).append(";");
				statsInfo.append("\tcreatedTime=").append(
						lastUsedDateFormat.format(new Date(createdTime)))
						.append(";");
				statsInfo.append("\tlastUsed=").append(
						(lastUsed == 0) ? "NA" : (lastUsedDateFormat
