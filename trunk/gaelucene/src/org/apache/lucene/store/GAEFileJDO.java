...
public class GAEFileJDO {
	// global logger
    private static final Logger log = Logger.getLogger(GAEFileJDO.class.getName());
...
//	}
    ...
    public static Long saveOrUpdate(String category, Long version, String fileName, Long fileLength, Long lastModified, Integer segmentCount) {
		PersistenceManager pm = PMF.get().getPersistenceManager();
		Query query = pm.newQuery(GAEFile.class);
		query.setFilter("cat == category && ver == version && name == fileName");
		query.declareParameters("String category, Long version, String fileName");
