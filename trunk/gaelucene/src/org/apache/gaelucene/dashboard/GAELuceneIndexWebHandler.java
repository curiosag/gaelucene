...
public class GAELuceneIndexWebHandler {
  ...
  public void processRegisterNewFile(HttpServletRequest request, HttpServletResponse response)
    ...
    log.info("receive a new file {cat:" + category + "; name:" + fileName + "; length:" + fileLength
        + "; lastModified:" + lastModified + "; segmentCount:" + segmentCount + "}.");
  ...
  public void processCommitNewFile(HttpServletRequest request, HttpServletResponse response)
      throws GAELuceneAuthException, IOException {
    GAELuceneOnlineUser onlineUser = userManager.getOnlineUser(request);
    GAELucenePermission permission = onlineUser.getPermission();
    if (!permission.canAdminSystem()) {
    ...
    //String action = ParamUtil.getString("do", "").trim();
    ...
    log.info("receive a new segment {fileId:" + fileId + "; segmentNo:" + segmentNo + "; segmentLength:"
        + segmentLength + "}.");
